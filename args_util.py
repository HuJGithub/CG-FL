...
def parse_args(args):
    ...
    if config_dict["-e"] == "fs" or config_dict["-e"] == "fs_cvae" or config_dict["-e"] == "lda_cvae":
        config_dict["-cp"] = find(args, "-cp")
        config_dict["-ep"] = find(args, "-ep")
    if config_dict["-e"] not in ["origin", "slice","slice_smote", "cc", "cc_smote","slice_ga", "ga", "slice_bsmote", "slice_cvae", "slice_mixup","resampling", "undersampling", "fs", "cvae", "fs_cvae", "smote","bsmote", "mixup", "lda_cvae","lda_smote"]:
